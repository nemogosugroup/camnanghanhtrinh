<template>
    <div class="app-container">  
        ...
        @pagination="fetch" />
        <dialog-mision :dialog-form-visible="dialogFormVisible" :dialog-status="dialogStatus" :formData="formData" :title-dialog="titleDialog" @hidedialog="handleHideDialog" @update-data="handleUpdateData"></dialog-mision>
...
<script>
    ...
    import DialogMision from './components/dialog'
    import AdminRepositoryFactory from '@/backend/respository';
    const CategoryMissionRepository = AdminRepositoryFactory.get('categoryMission');
    export default {
        name: 'CategoryMission',
        components: { Pagination, SearchPost, TableData, DialogMision , DialogRemove},
        data () {        
            return {
                list: [],
                listQuery: {
        ...
        methods: {
            async fetch(){
                this.isLoading = true;
                const { data } = await CategoryMissionRepository.list( this.listQuery );
            ...
            // remove
            async handleRemove(){
                const { data } = await CategoryMissionRepository.delete( this.id );
                if (data.success) {
                    this.$message({

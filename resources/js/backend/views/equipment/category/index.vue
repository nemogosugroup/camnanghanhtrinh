<template>
    <div class="app-container">  
        ...
        @pagination="fetch" />
        <dialog-equipment :dialog-form-visible="dialogFormVisible" :dialog-status="dialogStatus" :formData="formData" :title-dialog="titleDialog" @hidedialog="handleHideDialog" @update-data="handleUpdateData"></dialog-equipment>
...
<script>
    ...
    export default {
        ...
        components: { Pagination, SearchPost, TableData, DialogEquipment , DialogRemove},
        data () {        
            return {
                list: [],
                listQuery: {
        ...
        methods: {
            async fetch(){
                this.isLoading = true;
                const { data } = await CategoryEquipmentRepository.list( this.listQuery );
            ...
            handleShowDialog(data){              
                ...
                this.titleDialog = this.dialogStatus == 'create' ? 'Tạo danh mục mới' : 'Chỉnh sửa danh mục';
            ...
            // remove
            async handleRemove(){
                const { data } = await CategoryEquipmentRepository.delete( this.id );
                if (data.success) {
                    this.$message({
